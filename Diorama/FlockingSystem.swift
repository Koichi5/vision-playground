...
public struct FlockingSystem: System {
    ...
    let maxSpeed: Float = 0.001
    
    let relativeEntityName: String = "Root"
    let startTime = Date()
    ...
    public func update(context: SceneUpdateContext) {
        ...
        // Add forces togather to an accumulated acceleration table
        for bird in birds {
            ...
            var separationVelocityAccumulation = SIMD3<Float>(repeating: 0.0)
            var separationCount: Int = 0
            
            var alignmentVelocityAccumulation = SIMD3<Float>(repeating: 0.0)
            ...
            seekForce.clamp(lowerBound: SIMD3<Float>(repeating: -maxSeek), upperBound: SIMD3<Float>(repeating: maxSeek))
            ...
            if separationCount > 0 {
                ...
                if length(separationVelocity) > 0 {
                    ...
                    separationAcceleration.clamp(lowerBound: SIMD3<Float>(repeating: -maxSeparation), upperBound: SIMD3<Float>(repeating: maxSeparation))
